class Game:
    ...
    def advance_chapter(self, username):
        ...
        if self.database[username]['level'] > 0 and self.database[username]['level'] < 4:
            print("Level 2")
            print()
            print("Goodbye!")
            file = open('saves/new_user.txt', 'w', encoding='utf-8')
#             file.writelines(f"{self.database[username]['name']}, {self.database[username]['species']}, {self.database[username]['gender']}\n{self.database[username]['snack']}, {self.database[username]['weapon']}, {self.database[username]['tool']}\n{self.database[username]['level']} {self.database[username]['lives']}
# 2"self.database[username])
            quit()
